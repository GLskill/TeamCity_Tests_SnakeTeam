...
class MntStartedPage(BasePage):
    ...
    def check_first_start_page_open(self):
        with allure.step("Проверить что страница первого старта открыта"):
            expect(self.page_banner_on_step1).to_be_visible(timeout=WAIT_TIMEOUT)

    def check_db_page2_open(self):
        with allure.step("Проверить что страница изменилась на настройку базы"):
            expect(self.page_banner_on_step2_database).to_be_visible(timeout=WAIT_TIMEOUT)

    def check_license_agreement_page3_open(self):
        with allure.step("Проверить что страница изменилась на принятие лицензии"):
            expect(self.page_banner_on_step3_license_agreement).to_be_visible(timeout=WAIT_TIMEOUT)

    def check_page_banner_page4_open(self):
        with allure.step("Проверить что страница изменилась на настройку тимсити"):
            expect(self.page_banner_on_step4_teamcity_starting).to_be_visible(timeout=WAIT_TIMEOUT)
