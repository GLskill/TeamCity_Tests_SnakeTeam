...
class TestUiAdminCreateUser:
    ...
    def test_create_user_via_admin(self, auth_as_admin_and_open_user_page):
        users_page = auth_as_admin_and_open_user_page
        users_page.check_user_exists_in_list(users_page.user.username)

    @allure.id("81")
    @allure.title("UI — отмена создания пользователя")
    def test_cancel_create_user_via_admin(self, auth_as_admin_and_open_create_user_page, api_manager: ApiManager):
        auth_as_admin_and_open_create_user_page \
            .fill_username() \
            .fill_password_and_retype() \
            .click_cancel() \
            .check_users_page_opened()
        api_manager.user_steps.admin_get_deleted_user_by_username(auth_as_admin_and_open_create_user_page.username)

    @allure.id("82")
    @allure.title("E2E — пользователь созданный через API может залогиниться через UI")
    def test_user_created_via_api_can_login_via_ui(self, page, user_request, api_manager: ApiManager):
